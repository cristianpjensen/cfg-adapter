...
def main():
    ...
    metrics = evaluate(args.ref_batch, args.sample_batch)
...
def evaluate(ref_batch: str, sample_batch: str):
    """
    Args:
    ...
    metrics = {}
    metrics["inception_score"] = evaluator.compute_inception_score(sample_acts[0])
    metrics["fid"] = sample_stats.frechet_distance(ref_stats)
    metrics["sfid"] = sample_stats_spatial.frechet_distance(ref_stats_spatial)
    metrics["precision"], metrics["recall"] = evaluator.compute_prec_recall(ref_acts[0], sample_acts[0])
