...
from synthetic_dataset import SyntheticDiffusionSamplesDataset
...
def main(args):
    ...
    # Create model
    model = DiTCFGAdapter(depth=28, hidden_size=1152, patch_size=2, num_heads=16, input_size=args.latent_size, num_classes=args.num_classes)
    teacher_ckpt_path = f"pretrained_models/DiT-XL-2-{args.latent_size * 8}x{args.latent_size * 8}.pt"
    pretrained_state_dict = torch.load(teacher_ckpt_path, map_location=lambda storage, loc: storage, weights_only=True)

    # Load pretrained weights of base model (that are frozen)
    with torch.no_grad():
        for name, param in model.named_parameters():
            if name in pretrained_state_dict:
                param.copy_(pretrained_state_dict[name].to(device))

    model = DDP(model.to(device), device_ids=[rank])
    opt = torch.optim.AdamW(model.module.adapters.parameters(), lr=1e-4, weight_decay=0)
    model.train()
    ...
    dataset = SyntheticDiffusionSamplesDataset(args.data_path, num_timesteps=diffusion.num_timesteps)
    sampler = DistributedSampler(dataset, num_replicas=dist.get_world_size(), rank=rank, shuffle=True, seed=args.global_seed)
    loader = DataLoader(dataset, batch_size=args.global_batch_size // dist.get_world_size(), sampler=sampler, num_workers=args.num_workers, pin_memory=True, drop_last=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--latent-size", type=int, choices=[32, 64], default=32)
